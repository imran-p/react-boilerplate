...
const ref = new Firebase('https://fiery-torch-3626.firebaseio.com/');
...
class Inventory extends React.Component {
	...
	authHandler(err, authData) {
		...
		storeRef.on('value', (snapshot)=> {
			...
			// Claim store as our own there is no owner already
			if(!data.owner) {
				storeRef.set({
					owner : authData.uid
				});
	...
	renderInventory(key) {
		...
		return (
			<div className="fish-edit" key={key}>
				<input type="text" valueLink={linkState('fishes.' + key 
					+ '.name')} />
				<input type="text" valueLink={linkState('fishes.' + key 
					+ '.price')} />
